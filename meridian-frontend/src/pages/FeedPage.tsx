...
export function FeedPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <section className="border-b border-surface bg-white px-4 py-4 sm:px-6">
        <div className="flex gap-3">
          ...
          />
          <textarea
            className="min-h-20 flex-1 resize-none bg-transparent text-xl outline-none placeholder:text-[#536471]"
            placeholder="What's the latest patch?"
            aria-label="Create a post"
          />
        </div>
        <div className="mt-4 flex items-center justify-between pl-[60px]">
          <div className="flex gap-5 text-emerald-700">
          ...
          <button className="h-10 rounded-full bg-black px-7 font-bold text-white hover:bg-verified hover:text-black">Post</button>
        </div>
      </section>

      <div className="bg-white">
        {posts.map((post) => (
          <article key={post.id} className="x-post-font border-b border-surface px-4 py-3 transition hover:bg-neutral-50 sm:px-6">
            <div className="flex items-start gap-3">
              <img src={post.avatar} alt="" className="h-12 w-12 shrink-0 rounded-full object-cover grayscale" />
              <div className="min-w-0 flex-1">
                <div className="flex min-w-0 items-start gap-2">
                  <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-1 text-[15px] leading-5">
                  ...
                  <button className="grid h-8 w-8 place-items-center rounded-full text-[#536471] hover:bg-surface hover:text-ink" aria-label="More actions">
                    <MoreHorizontal size={19} />
                  </button>
                </div>

                <div className="mt-1 flex flex-wrap items-center gap-2 text-[13px] leading-5">
                ...
                {post.lineage && <p className="mt-2 font-mono text-xs text-muted">Fork lineage: {post.lineage.join(' / ')}</p>}

                <p className="x-post-body mt-2 max-w-[660px] text-[#0f1419]">{post.excerpt}</p>

                {post.code && (
                  <pre className="code-block thin-scrollbar mt-3 max-w-[660px] overflow-x-auto p-4 font-mono text-[13px] leading-6">
                    <code>{post.code}</code>
                  </pre>
                )}
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {post.tags.map((tag) => (
                    <span key={tag} className="rounded-full bg-surface px-2.5 py-1 text-xs font-medium text-[#536471]">
                      [{tag}]
                    </span>
                  ))}
                </div>

                <details className="mt-3 max-w-[660px] rounded-md border border-surface p-3 text-sm">
                  <summary className="cursor-pointer font-semibold">Patch history timeline</summary>
                  <ol className="mt-3 space-y-2 text-neutral-600">
                    <li>v2.4 accepted benchmark correction by @kernel_notes</li>
                    <li>v2.3 merged epoll fallback note by @ops-lab</li>
                  </ol>
                </details>

                <div className="mt-3 flex max-w-[620px] items-center justify-between text-[13px] text-[#536471]">
