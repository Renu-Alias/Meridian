...
### 4.4 USP 4 — Impact-Based Monetization
**Description:** Writers earn micro-payments based on demonstrated impact — bookmarks, internal shares, and "Used This At Work" reactions — rather than raw pageviews.
 
**User Stories**
- As an author, I want to be paid when my writing is actually applied, not just viewed, so my incentives match producing genuinely useful content.
- As a reader, I want a simple way to signal that a post helped me at work.
**Functional Requirements**
- FR-4.1: System shall record Bookmark, Share Internally, and "Used This At Work" reactions per post, per user.
- FR-4.2: System shall calculate author earnings based on a defined weighting of these reactions and credit them to an in-app wallet.
- FR-4.3: System shall provide a monthly payout dashboard showing earnings by post and by reaction type.
- FR-4.4: System shall integrate with Stripe Connect for author payout to external bank accounts.
- FR-4.5: System shall rate-limit reactions per reader per post to reduce trivial gaming of the earnings model.
**Acceptance Criteria**
- Triggering a "Used This At Work" reaction updates the author's wallet balance within the same billing cycle.
- An author can view a breakdown of earnings by post and reaction type in the payout dashboard.
**Dependencies:** Stripe SDK, wallet ledger data model, Celery payout jobs.
